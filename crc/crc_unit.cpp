...
uint16_t CRC::rtuCRC16(const void * data, size_t len)
{
    if (len > 256) return 0;
...
uint32_t CRC::crc32(const void * data, size_t len)
{
    uint32_t CRC32 = uint32_t(-1);// 0xffffffff;
    ...
    if(buf && len)
    {
     ...
     while(buf < buf_end)
     {
       CRC32 = (CRC32 >> 8) ^ CRC32Table[(CRC32 ^ (*buf++))&0xFF];
     }
    }
    return CRC32^uint32_t(-1);
}


/**
  * @brief  Вычисляет 8-bit CRC используя предыдущий результат и новое значение
  *         CRC8 вычисляется по алгоритму аналогичному вычислению CRC в протоколе 1-wire.
...
unsigned char crc8(uint8_t data, uint8_t crc)
{
...
uint8_t CRC::crc8_1wire(const void *data, size_t len)
{
    uint8_t crc = 0;
    const uint8_t *ptr = reinterpret_cast<const uint8_t *>(data);
    const uint8_t *ptr_end = ptr+len;
