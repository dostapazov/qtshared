...
{
#ifndef Q_OS_ANDROID
  if(!cbBaud->count())  init_serial_bauds();
#endif
...
void mutli_iodev_params::set_serial_params (const QString & param_str)
{
  ...
  QString  name  ;
  QString  params;
  if(sl.count()>0) name   = sl[0].trimmed();
  if(sl.count()>1) params = sl[1].trimmed();
  if(!dev_name->count()) scan_serial_port();
  ...
  for(int i =0 ;i < sl.count(); i++)
  {
    ...
    switch(i)
    {
      case 0: set_serial_bauds(val);
              break;
      case 1: cbDataSize->setCurrentIndex(( val == QSerialPort::Data7 )? 1 : 0 ) ;
              break;
      case 2: sbStopBits->setValue( (val == QSerialPort::TwoStop) ? 2 : 1);
              break;
      case 3:
      switch(val)
      {
        case QSerialPort::EvenParity : this->rb_parity_even->setChecked(true);break;
        case QSerialPort::OddParity  : this->rb_parity_odd ->setChecked(true);break;
        default : rb_parity_none->setChecked(true);break;
      }
      break;
      case 4:
        switch(val)
        {
         case QSerialPort::SoftwareControl : rb_flow_serial_software->setChecked(true);break;
         case QSerialPort::HardwareControl : rb_flow_serial_hardware->setChecked(true);break;
         default : rb_flow_serial_none->setChecked(true);break;
        }
      break;
...
void mutli_iodev_params::set_ftdi_params   (const QString & param_str)
{
    ...
    for(int i =0 ;i < sl.count(); i++)
    {
      ...
      switch(i)
      {
        case 0: set_serial_bauds(val);
        break;
        case 1: cbDataSize->setCurrentIndex( (val == int(FT_BITS_7)) ? 1 : 0 ) ;
        break;
        case 2: this->sbStopBits->setValue( (val == int(FT_STOP_BITS_2)) ? 2 : 1);
        break;
        case 3:
        switch(val)
        {
          case FT_PARITY_EVEN : this->rb_parity_even->setChecked(true);break;
          case FT_PARITY_ODD  : this->rb_parity_odd ->setChecked(true);break;
          default : rb_parity_none->setChecked(true);break;
        }
        break;
        case 4:
          switch(val)
          {
           case FT_FLOW_DTR_DSR  : rb_flow_dtr_dsr->setChecked(true);break;
           case FT_FLOW_RTS_CTS  : rb_flow_rts_cts->setChecked(true);break;
           case FT_FLOW_XON_XOFF : rb_flow_xon_xof->setChecked(true);break;
           default : rb_flow_none->setChecked(true);break;
          }
        break;
...
QAbstractButton *mutli_iodev_params::interface_button(QMultiIODev::conn_types_t iface_type)
{
  ...
  switch(iface_type)
  {
    case QMultiIODev::tcp_ip  : btn = tbTcp;break;
    case QMultiIODev::udp     : btn = tbUdp;break;
#ifndef Q_OS_ANDROID
    case QMultiIODev::serial  : btn = tbSerial;break;
    case QMultiIODev::ftdi    : btn = tbSerial;break;
#endif
    default : break;
  }
 return btn ;
