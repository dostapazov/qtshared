...
QIODevice* QMultiIODev::do_create_device(const QString& dev_params)
{
	...
	if (sl.size() > 1)
	{
		...
		{
			case tcp_ip :
				iodev = create_tcp_socket(sl[1].trimmed());
				break;
			case udp    :
				iodev = create_udp_socket(sl[1].trimmed());
				break;
			case ftdi   :
				iodev = create_ftdi(sl[1].trimmed());
				break;
			case serial :
				iodev = create_serial(sl[1].trimmed());
				break;
			default     :
				return Q_NULLPTR;
		}
...
void QMultiIODev::init_socket(QAbstractSocket* socket, const QString& params )
{
	...
	if (sl.count() > 1)
		socket->setProperty(prop_dev_params, sl[1].trimmed());
}
...
QIODevice* QMultiIODev::create_serial(const QString& str)
{
	...
	QStringList params;
	if (sl.count() > 1)
		params = sl[1].trimmed().split(QLatin1Char(','));
