...
static PtrIsAppThemed pIsAppThemed = Q_NULLPTR;
static PtrGetCurrentThemeName pGetCurrentThemeName = Q_NULLPTR;
...
public:
    QtDotNetStylePrivate()
    {
        ...
        if (themeLib.isLoaded()) {  //resolve uxtheme functions
            pIsAppThemed            = reinterpret_cast<PtrIsAppThemed>        (themeLib.resolve("IsAppThemed"));
            pGetCurrentThemeName    = reinterpret_cast<PtrGetCurrentThemeName>(themeLib.resolve("GetCurrentThemeName"));
        }
    }
    ~QtDotNetStylePrivate()
    {}
...
inline QColor QtDotNetStylePrivate::highlightOutlineColor(const QStyleOption *option, bool act = false) const {
    ...
    default:
        if (act)
            color = option->palette.highlight().color().darker(110);
        else
            color = option->palette.highlight().color().lighter(110);
        break;
    }
    return color;
}
...
: QProxyStyle(QStyleFactory::create("WindowsVista")), d(new QtDotNetStylePrivate())
{
    ;
    d->theme = Office;
                                    ...
                                    QPainter *painter, const QWidget *widget) const
{
    ...
    case PE_PanelButtonTool:
        {
            ...
            if (QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA && QSysInfo::WindowsVersion < QSysInfo::WV_NT_based)
                QProxyStyle::drawPrimitive(element, &opt2, painter, widget);
            else
                QProxyStyle::drawPrimitive(element, &opt2, painter, widget);
        }
        break;
    ...
    case PE_PanelMenuBar:
            ...
            painter->setBackground(QBrush(QColor("#ffffde")));
        break;
    default:
        if (QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA && QSysInfo::WindowsVersion < QSysInfo::WV_NT_based)
            QProxyStyle::drawPrimitive(element, option, painter, widget);
    ...
    case CE_MenuItem:
        ...
        case CE_MenuBarItem:
            if (const QStyleOptionMenuItem *mbi = qstyleoption_cast<const QStyleOptionMenuItem *>(option))
            {
                ...
                mbiCopy.palette = pal;
                QProxyStyle::drawControl(element, &mbiCopy, painter, widget);
        }
            break;
        ...
        case CE_DockWidgetTitle:
            if (const QStyleOptionDockWidget *dockWidget = qstyleoption_cast<const QStyleOptionDockWidget *>(option)) {
                ...
                bool verticalTitleBar = v2 == Q_NULLPTR ? false : v2->verticalTitleBar;
        ...
        case CC_ComboBox:
            if (widget && qobject_cast<QToolBar *>(widget->parentWidget())) {
                ...
                if (const QStyleOptionComboBox *cmb = qstyleoption_cast<const QStyleOptionComboBox *>(option))
                {
                    ...
                    QRect editRect = QProxyStyle::subControlRect(CC_ComboBox, cmb, SC_ComboBoxEditField, widget).adjusted(0, 0, 3, 1);
                    ...
                    if (cmb->subControls & SC_ComboBoxArrow) {
                        ...
                        QRect ar = QProxyStyle::subControlRect(CC_ComboBox, cmb, SC_ComboBoxArrow, widget).adjusted(1, 0, -1, 0);
                        if (active) {
                            painter->save();
                        ...
                        QProxyStyle::drawPrimitive(PE_IndicatorArrowDown, &arrowOpt, painter, widget);
                    }
                }
            } else {
                if (QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA && QSysInfo::WindowsVersion < QSysInfo::WV_NT_based)
        ...
        if (const QStyleOptionToolButton *toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(option))
        {
            ...
            if ((widget && qobject_cast<QToolBar*>(widget->parentWidget())) &&
                ...
                if (toolbutton->subControls & SC_ToolButtonMenu) {
                    ...
                    tool.rect = QProxyStyle::subControlRect(control, toolbutton, SC_ToolButtonMenu, widget);
                    painter->setPen(d->highlightOutlineColor(option, act));
                                        ...
                                        const QSize &size, const QWidget *widget) const
{
    QSize newSize;
    if (QSysInfo::WindowsVersion >= QSysInfo::WV_VISTA && QSysInfo::WindowsVersion < QSysInfo::WV_NT_based)
        newSize = QProxyStyle::sizeFromContents(type, option, size, widget);
    else
        newSize = QProxyStyle::sizeFromContents(type, option, size, widget);
...
void QtDotNetStylePrivate::updatePalette()
{
    ...
    default:
        ...
        menuBackgroundColor.setHsv(menuBackgroundColor.hue(), 3 * menuBackgroundColor.saturation()/8 ,250);
        break;
    }
}
